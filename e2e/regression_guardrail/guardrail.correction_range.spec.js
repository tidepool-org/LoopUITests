...
describe('guard rail correction range', () => {
    ...
    it('maximum 121 can be set, warning', async () => {
        await loop.screens.settings.SetCorrectionRanges([{ time: '12:00 AM', min: '120', max: '121' }]);
        //TODO assert on warning
    });
    it('maximum 120 can be set, no warning', async () => {
        await loop.screens.settings.SetCorrectionRanges([{ time: '12:00 AM', min: '119', max: '120' }]);
    });
    it('minimum 70 can be set, no warning', async () => {
        await loop.screens.settings.SetCorrectionRanges([{ time: '12:00 AM', min: '70', max: '180' }]);
    });
    it('minimum 69 can be set, warning', async () => {
        await loop.screens.settings.SetCorrectionRanges([{ time: '12:00 AM', min: '69', max: '180' }]);
        //TODO assert on warning
    });
    it('minimum 60 can be set, warning', async () => {
        await loop.screens.settings.SetCorrectionRanges([{ time: '12:00 AM', min: '60', max: '180' }]);
        //TODO assert on warning
    });
